...
public class CreateProfileAPIAsyncTask extends AsyncTask<String, Void, UserProfiles > {
    ...
    private UserProfiles parseJSON(String s){
        ...
        try{
            ...
            UserProfiles userProfiles = new UserProfiles(username,password,firstName,lastName,location,department,position,story,admin,pointsToAward,imageBytes,rewards);
            return userProfiles;
        }
        catch (Exception e) {
            e.printStackTrace();
