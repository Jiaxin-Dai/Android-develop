...
public class UpdateProfileAPIAsyncTask extends AsyncTask<String,Void,String > {
    ...
    private UserProfiles parseJSON(String s) {
        ...
        try {
            ...
            String reward = juser.getString("rewardRecords");
            JSONArray jReward = new JSONArray(reward);

            List<Rewards> rewardsList = new ArrayList<>();
            if(jReward.length()>0){
                for(int i = 0;i<jReward.length();i++) {
                    JSONObject jRewards = (JSONObject)jReward.get(i) ;
                ...
                rewardsList = null;
            UserProfiles user = new UserProfiles(username,password,firstName,lastName,location,department,position,story,admin,pointsToAward,imageBytes,rewardsList);
            return user;
