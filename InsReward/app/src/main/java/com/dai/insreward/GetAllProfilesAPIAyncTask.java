...
//               jsonRe = jsonUser.getJSONArray("rewards");
//
//                List<Rewards> rewardsList = new ArrayList<>();
//                if(jsonRe.length()>0){
//                    for(int j = 0;j<jsonRe.length();j++) {
//                        JSONObject jRewards = (JSONObject)jsonRe.get(i) ;
...
//                    rewardsList = null;
//
//                userProfiles = new UserProfiles(username, firstName, lastName, location, department, position, story, admin, pointsToAward, imageBytes,rewardsList);
