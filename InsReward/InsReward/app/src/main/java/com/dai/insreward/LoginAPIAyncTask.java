...
public class LoginAPIAyncTask extends AsyncTask<String, Void, UserProfiles> {
    ...
    private UserProfiles parseJSON(String s){
        try{
            ...
            UserProfiles userProfiles = new UserProfiles(username,password,oldPassword,firstName,lastName,location,department,position,story,admin,pointsToAward,imageBytes,rewardsList);
            return userProfiles;
        }
        catch (Exception e) {
            e.printStackTrace();
